...
PowerShell: `Copy-Item task\log_slice.md.template log_slice.md`. Do not modify `task/case.log`.
...
4. Verify every quoted line with `grep -F` or PowerShell `Select-String -SimpleMatch`; re-copy any missing or partial line from the log.
5. Commit and push `log_slice.md`, then paste its complete contents into the task 1.11 LMS AI-check box.

## Rules and submission check

- Every non-empty line inside a code fence must be one complete verbatim line from `case.log`; edge whitespace does not matter.
- Identical errors and first project frames across tests may represent one shared cause; do not duplicate the same slice.
- Do not change a test expectation merely to match the observed value; confirm the product contract first.
...
- Paste only `log_slice.md` into the LMS AI check, never the full `case.log`.
